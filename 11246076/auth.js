...
function initAuthGate() {
    ...
    const mainPages = ['index.html', 'consulting.html', 'dm.html', 'budget.html', 'policy.html', 'community.html',
...
function _injectHeaderStyles() {
    ...
    style.textContent = `
        ...
        .auth-plan-pill.free:hover { background: rgba(255,255,255,0.18); }
        .auth-plan-pill.basic {
            background: rgba(255,215,0,0.15);
            color: #FFD700;
        }
        .auth-plan-pill.basic:hover { background: rgba(255,215,0,0.25); }
        .auth-plan-pill.pro {
            background: linear-gradient(135deg, rgba(155,89,182,0.2), rgba(240,147,251,0.2));
            color: #f093fb;
        }
        .auth-plan-pill.pro:hover { background: linear-gradient(135deg, rgba(155,89,182,0.3), rgba(240,147,251,0.3)); }
        .auth-avatar-link {
            display: flex;
            align-items: center;
            gap: 0.4rem;
        ...
        .auth-avatar-link:hover { opacity: 0.85; }
        .auth-avatar-circle {
...
function injectHeaderInfo() {
    ...
    bar.innerHTML = `
        ...
        <a href="wallet.html" class="auth-coin-pill" id="auth-coins" title="前往錢包">
            🪙 <span id="auth-coins-val">${data.coins}</span>
        </a>
        <a href="subscription.html" class="auth-plan-pill ${level}" id="auth-plan" title="查看方案">
            ${levelNames[level] || 'Free'}
        </a>
        <a href="profile.html" class="auth-avatar-link" title="個人檔案">
